...
output %>%
  filter(is.na(error)) %>%
  select(-error) %>%
  mutate(type = ifelse(grepl(args[1], from), "out", "in"),
  ...
  filter(grepl("out", paste0(type, collapse = "")) & grepl("in", paste0(type, collapse = ""))) %>% 
  ungroup() %>%
  mutate(subject = factor(subject, labels = length(levels)),
